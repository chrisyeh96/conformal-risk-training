...
TRAIN_FRAC = 0.65        # of the training+calib data, use a 65% train / 35% calib split
...
def get_tensors(
    shuffle: bool, log_prices: bool, future_temp: bool = True,
    label_noise: float = 0.
    ...
    if label_noise > 0:
        rng = np.random.default_rng(seed=100)
        Y += rng.normal(0, label_noise, Y.shape)  # add noise to prices

    if not future_temp:
        # features 72-95 (inclusive) are future temperature features
        X = np.concatenate([X[:, :72], X[:, 96:]], axis=1)
